...
class SliderByItchief {
  ...
  init($elem, config) {
    const $wrapper = $elem.querySelector('.slider__wrapper');
    const $items = $elem.querySelectorAll('.slider__item');
    const $controls = $elem.querySelectorAll('.slider__control');
    const $controlLeft = $elem.querySelector('.slider__control_left');
    const $controlRight = $elem.querySelector('.slider__control_right');
    const _wrapperWidth = parseFloat(getComputedStyle($wrapper).width); // ширина обёртки
    const _itemWidth = parseFloat(getComputedStyle($items[0]).width); // ширина одного элемента
    ...
    var _transformItem = function (direction) {
      ...
      $wrapper.style.transform = 'translateX(' + _transform + '%)';
    };
